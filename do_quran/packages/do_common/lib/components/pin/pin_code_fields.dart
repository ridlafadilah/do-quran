...
import 'package:do_common/components/pin/cursor_painter.dart';
import 'package:do_common/components/pin/models/platform.dart';

part 'models/dialog_config.dart';
...
class PinCodeTextField extends StatefulWidget {
  ...
  /// Auto dispose the [controller] and [FocusNode]
  /// upon the destruction of widget from the widget tree. Default is true
...
class _PinCodeTextFieldState extends State<PinCodeTextField>
  ...
  void initState() {
    // if (!kReleaseMode) {
    //   print(
    //       "IF YOU WANT TO USE COLOR FILL
    //        FOR EACH CELL THEN SET enableActiveFill = true");
    // }
      ...
      if (widget.enabled && _inputList.join('') != currentText) {
        if (currentText.length >= widget.length) {
          if (widget.onCompleted != null) {
            ...
            //  delay the onComplete event handler
            // to give the onChange event handler enough time to complete
  ...
  void dispose() {
    if (widget.autoDisposeControllers) {
      ...
      // if (!kReleaseMode) {
      //   print(
      //       "*** Disposing _textEditingController
      //        and _focusNode, To disable this feature please set
            ...
            AbsorbPointer(
              ...
              child: AutofillGroup(
                child: TextFormField(
                  ...
                  // using same as background color
                  // so that it can blend into the view
                  cursorWidth: 0.01,
